...
def denseunet_3d(input_size=(256, 256, 256, 1)):

    #  ************************3d volume input******************************************************************

    lr=1e-4
    args=argument(input_size[0])
    inputs = Input(batch_shape=(1,)+input_size, name='input_data')
    loss = 'binary_crossentropy'

    #  ************************(batch*d3cols)*2dvolume--2D DenseNet branch**************************************
    input2d = Lambda(slice, arguments={'h1': 0, 'h2': 2})(inputs)
    single = Lambda(slice, arguments={'h1':0, 'h2':1})(inputs)
    input2d = concatenate([single, input2d], axis=3)
    for i in range(args.input_cols - 2):
        input2d_tmp = Lambda(slice, arguments={'h1': i, 'h2': i + 3})(inputs)
        input2d = concatenate([input2d, input2d_tmp], axis=0)
        if i == args.input_cols - 3:
            final1 = Lambda(slice, arguments={'h1': args.input_cols-2, 'h2': args.input_cols})(inputs)
            final2 = Lambda(slice, arguments={'h1': args.input_cols-1, 'h2': args.input_cols})(inputs)
    ...
    input3d_ori = Lambda(slice, arguments={'h1': 0, 'h2': args.input_cols})(inputs)
    ...
    final_ac = Activation('sigmoid', name='final_ac')(final_bn)
    classifer = Conv3D(2, (1, 1, 1), padding="same", name='2d3dclassifer')(final_ac)


    model = Model( inputs = inputs, outputs = classifer, name='auto3d_residual_conv')
